...
const Render = (): JSX.Element => {
    ...
    const handleUploadFile = async () => {
        if(selectedFile.size) {
            ...
            const option = {
                url: `${process.env.REACT_APP_URL_API}/api/convert/fingerprint-raw`,
                data: formData,
                timeout: 0,
            ...
            await postAxios(option).then(res => {
                ...
                console.log('finish')
                const attendanceWS = xlsx.utils.json_to_sheet(res.data.data);
                ...
                FileSaver.saveAs(finalData, `formated_${file.name}_${moment().format("DD/MM/YYYY hh:mm:ss")}.xlsx`);
            }).then(res => {
            })
        }
    }
    ...
    const result = (
        <>
            ...
            <Panel>
                <PanelHeader>
                    <Typography className="titleForm" variant='h5'>Reformat Exported File from Fingerprint Attendance Machine</Typography>
                </PanelHeader>
